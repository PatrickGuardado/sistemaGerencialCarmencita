  ...
  <div class="ventas_producto flex flex-col">
    ...
    <div class="flex justify-between items-center px-4 text-black">
      <div>
        ...
        <input id="fecha-inicio" type="date" v-model="fechaInicio" class="ml-2 border border-gray-300 rounded px-3 py-2" @change="calcularFechaFin">
        <label for="fecha-fin" style="padding-left:1rem;"><strong>Hasta</strong></label>
        <input id="fecha-fin" type="date" v-model="fechaFin" class="ml-2 border border-gray-300 rounded px-3 py-2" disabled>
      </div>
...
export default {
  ...
  created() {
    ...
    // Calculamos la fecha de fin (desde el lunes al domingo de la semana que contiene la fecha de inicio)
  ...
  methods: {
    ...
    calcularFechaFin() {
      ...
      fechaInicioDate.setDate(fechaInicioDate.getDate());
      const diaSemana = fechaInicioDate.getDay(); // 0 es domingo, 1 es lunes, ..., 6 es sábado

      // Calcular el lunes de la misma semana
      const lunesOffset = diaSemana === 0 ? -6 : 0 - diaSemana; // Si es domingo, restar 6, si no, restar el día de la semana menos 1
      const domingoOffset = diaSemana === 0 ? 0 : 6 - diaSemana; // Si es domingo, el offset es 0, si no, 7 - día de la semana

      const lunes = new Date(fechaInicioDate);
      lunes.setDate(fechaInicioDate.getDate() + lunesOffset);
      const domingo = new Date(fechaInicioDate);
      domingo.setDate(fechaInicioDate.getDate() + domingoOffset);

      this.fechaInicio = lunes.toISOString().substr(0, 10);
      this.fechaFin = domingo.toISOString().substr(0, 10);
    }
